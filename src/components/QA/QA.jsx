...
подробно об этом в разделе сайта «Доставка».
                
`;
const text2 = `

автозапчасти, произведенные сторонним производителем
Запчасти – дубликаты, или неоригинальные запчасти – это
Нередко такие производители поставляют запчасти и на конвейер
автозапчастей, не являющимся производителем Вашего автомобиля.
автозапчасти только проверенных и надежных производителей,
производителя, и в розничные магазины в собственной упаковке.
которые выгодно отличаются по цене от оригинальных запчастей.
Ничего страшного в этом нет. Detal.kg продает неоригинальные
 Искать дубликаты оригинальных запчастей позволяет наша база
 кроссов – переходов с каталожного номера запчасти одного
 производителя на номер запчасти другого производителя,
 являющийся аналогом. Неоригинальные запчасти для Вашего
 автомобиля позволяют сэкономить существенную сумму денег при
 покупке, не экономя на качестве. О качестве и сроке службы
 неоригинальных запчастей того либо производителя Вы можете
...
const QA = () => (
  <div className="container qa">
    ...
    <Collapse accordion>
      <Panel header="Какие сроки выполнения заказа после его оплаты?" key="1">
        <p>{text}</p>
      </Panel>
      <Panel
